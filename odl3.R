...
for (i in seq(1,n_species)) {
  ...
  for (j in seq(i,n_species)) {
    ...
    # put sequences into DNA string set
#    si <- DNAString(sequence_i)
#    sj <- DNAString(sequence_j)
#    s12 <- DNAStringSet(c(sequence_i,sequence_j))
    s12 <- c(sequence_i, sequence_j)
    saln <- msa(DNAStringSet(s12))
    salntf <- table(as.matrix(saln)[1,] == as.matrix(saln)[2,])
    percent_similarity <- salntf[2]/dim(saln)[2]
