...
# Functions
# 1. sequence similarity between two genomes in a sequence alignment
calc_sequence_similarity <- function(sequence_alignment, i, j) {
  seqi <- as.matrix(sequence_alignment)[i,]
  seqj <- as.matrix(sequence_alignment)[j,]
  bp_match <- 0
  bp <- 0
  for (k in seq(1,length(seqi))) {
    if ((seqi[k] == seqj[k]) && (!(seqi[k] == "-"))) {
      bp_match <- bp_match + 1
    }
    if (!((seqi[k] == "-") && (seqj[k] == "-"))) {
      bp <- bp + 1
  ...
  percent_similarity <- bp_match / bp
  percent_similarity
}
...
max_n_genes <- 300 # > 150, since plastids have ~150 genes
...
# create folder for GenBank files
# These files can be sent to GenomVx
dir.create("GenBank")
# set counter to zero
c <- 0
...
for (accession in acclist$accession) {
  ...
  for (i in 1:num_genes) {
    if (!is.na(annot$product[i])) {
      ...
      gene_matrix[species,j] = annot$product[i]
      gene_start_matrix[species,j] = annot$start[i]
      gene_end_matrix[species,j] = annot$end[i]
    }
  }
  # list of genes/RNAs for species
  print(paste("Writing elements for ",species,"...",sep=""))
  write(paste(">",species,sep=""), file=paste("species_products.txt"), sep="\t", append = T)
  # go through all products and add them to sp_genes_rnas and write them into a file
  species_genes_rnas <- ""
  n_species_genes_rnas <- 0
  for (pr in annot$product) {
    if (!is.na(pr)) {
      n_species_genes_rnas <- n_species_genes_rnas + 1
      if (n_species_genes_rnas == 1) {
        species_genes_rnas <- gsub(" ","_",pr)
      } else if (n_species_genes_rnas > 1) {
        species_genes_rnas <- paste(species_genes_rnas,gsub(" ","_",pr),sep="\t")
      }
    }
  }
  ...
  Sys.sleep(1)
  # save GenBank file
  gbfile <- paste("GenBank/",species,".gb",sep="")
  write.dna(GBi,gbfile)
  ...
  acclist[acclist$accession==accession,]$GC <- (acgt['c']+acgt['g'])/sequence_length
}
...
for (i in seq(1,n_species)) {
  ...
  sequences <- c(sequences, sequence_i)
  ...
  for (j in seq(i,n_species)) {
    ...
    if (i < j) {
      ...
      d <- 0
      # get gene products for species i, and exclude NA elements
...
print("Calculating sequence similarity...")

sequence_alignment <- msa(DNAStringSet(sequences))

for (i in seq(1,n_species)) {
  species_i <- acclist[i,]$species
  sequence_similarity_matrix[species_i,species_i] = 1
  for (j in seq(i,n_species)) {
    species_j <- acclist[j,]$species
    if (i < j) {
      percent_similarity <- calc_sequence_similarity(sequence_alignment, i, j)
...
write.table(sequence_similarity_matrix, file="sequence_similarity_matrix.txt", sep="\t", quote = F)
...
cexx <- length(species)/(2*length(species))
ceyy <- cexx
...
# make heat map
sequence_similarity_heatmap_name="heatmap_sequence_similarity.jpg"
...
header = "cluster\tno. species\tMean DNA length±SEM\tMean GC%±SEM\tmin\tmean\tmax\tSEM\tp-value\tneglog"
...
for (n_cluster in 1:k_sequence_similarity) {
  ...
  if (csize >= 2) {
    ...
    sem_dna_length <- sprintf("%.3f",sd(acclist[ctk_sequence_similarity == n_cluster,]$sequence_length)/sqrt(csize))

    mean_gc <- sprintf("%.3f",mean(acclist[ctk_sequence_similarity == n_cluster,]$GC))
    sem_gc <- sprintf("%.3f",sd(acclist[ctk_sequence_similarity == n_cluster,]$GC)/sqrt(csize))
    ...
    statistics = paste(n_cluster, nn, paste(mean_dna_length,"±",sem_dna_length,sep=""), paste(mean_gc,"±",sem_gc,sep=""), min2, mean2, max2, sd2, p_value, neglog2, sep="\t")
    statistics2 = gsub("\n","\t",statistics)
...
# make heat map
similarity_heatmap_name="heatmap_similarity.jpg"
...
for (n_cluster in 1:k_similarity) {
  ...
  if (csize >= 2) {
    ...
    sem_dna_length <- sprintf("%.3f",sd(acclist[ctk_similarity == n_cluster,]$sequence_length)/sqrt(csize))

    mean_gc <- sprintf("%.3f",mean(acclist[ctk_similarity == n_cluster,]$GC))
    sem_gc <- sprintf("%.3f",sd(acclist[ctk_similarity == n_cluster,]$GC)/sqrt(csize))
...
# make heat map
combined_heatmap_name="heatmap_combined.jpg"
...
cluster_sizes <- table(ctk_combined)
non_groups = c()
...
for (n_cluster in 1:k_combined) {
  ...
  if (csize >= 2) {
    ...
    sem_dna_length <- sprintf("%.3f",sd(acclist[ctk_combined == n_cluster,]$sequence_length)/sqrt(csize))

    mean_gc <- sprintf("%.3f",mean(acclist[ctk_combined == n_cluster,]$GC))
    sem_gc <- sprintf("%.3f",sd(acclist[ctk_combined == n_cluster,]$GC)/sqrt(csize))
...
par(mar=c(0,0,0,0))
legend('topleft',legend=names(gene_colors),fill=gene_colors,cex=12,ncol=n_legend_columns,bty='o')
dev.off()
